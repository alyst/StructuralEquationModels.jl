...
model_ml_ridge = Sem(SemML(observed, RAM(spec)), SemRidge(spec, 16:20) => 0.001)

model_ml_const = Sem(SemML(observed, RAM(spec)), SemConstant(3.465))

model_ml_weighted = Sem(SemML(observed, RAM(spec)) => n_obs(observed))
