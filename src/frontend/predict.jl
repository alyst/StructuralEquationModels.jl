function predict(model::SemLoss, params::AbstractVector,
    ...
    score_var_inds = !isnothing(score_vars) ? check_var_indices(ram, score_vars, normalize=false) : nothing
...
"""
function (transform::SemVariablesTransform)(
    scores::AbstractMatrix;
    inverse::Bool = false,
)
    size(scores, 2) == length(transform.vars) || throw(DimensionMismatch(
...
function (transform::SemVariablesTransform)(
    ...
    length(scores) == length(transform.vars) || throw(DimensionMismatch(
...
function score_basis_transform(
    ...
    alpha >= 0 || throw(ArgumentError("The regularization parameter alpha must be non-negative"))
    isnothing(prior_cov_alpha) || prior_cov_alpha >= 0 || throw(
        ArgumentError("The regularization parameter prior_cov_alpha must be non-negative"),
    )

    implied = imply(model)
    ram = implied.ram_matrices
    lvar_inds = check_var_indices(ram, latent_vars, allow_observed = false, normalize = true)
    ...
    base_solver = QRScoresSolver(implied, lvar_inds, A, S, lv_I_A⁻¹;
                                 alpha, prior_cov_alpha = 0)
    nobs = nobserved_vars(implied)
