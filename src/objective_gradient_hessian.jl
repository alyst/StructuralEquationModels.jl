...
(::EvaluationTargets{true,true,false})(objective, gradient, hessian) = (objective, gradient)
(::EvaluationTargets{true,false,true})(objective, gradient, hessian) = (objective, hessian)
(::EvaluationTargets{false,true,true})(objective, gradient, hessian) = (gradient, hessian)
(::EvaluationTargets{true,true,true})(objective, gradient, hessian) = (objective, gradient, hessian)
...
function update!(targets::EvaluationTargets, imply::SemImply, params)
    is_objective_required(targets) && update_objective!(imply, params)
    is_gradient_required(targets) && update_gradient!(imply, params)
    is_hessian_required(targets) && update_hessian!(imply, params)
end
...
objective_type(objective::Nothing, gradient::AbstractArray{T}, hessian) where T <: Number = T
objective_type(objective::Nothing, gradient::Nothing, hessian::AbstractArray{T}) where T <: Number = T
...
function evaluate!(objective, gradient, hessian, model::Sem, params)
    ...
    # update imply states of all SemLoss terms before term calculation loop
    # to make sure all terms use updated imply states
    targets = EvaluationTargets(objective, gradient, hessian)
    for term in loss_terms(model)
        issemloss(term) && update!(targets, imply(term), params)
    end

    for term in loss_terms(model)
        t_obj = evaluate!(objective, t_grad, t_hess, term, params)
        #@show nameof(typeof(term)) t_obj
        objective = accumulate_loss!(objective, gradient, hessian, weight(term), t_obj, t_grad, t_hess)
    end
    return objective
end

# internal function to accumulate loss objective, gradient and hessian
function accumulate_loss!(total_objective, total_gradient, total_hessian,
                          weight::Nothing, objective, gradient, hessian
)
...
function accumulate_loss!(total_objective, total_gradient, total_hessian,
                          weight::Number, objective, gradient, hessian
)
...
# update imply state, its gradient and hessian
function _evaluate!(objective, gradient, hessian, loss::SemLoss, params)
    # note that any other Sem loss terms that are dependent on imply
    # should be enumerated after the SemLoss term
    # otherwise they would be using outdated imply state
    update!(EvaluationTargets(objective, gradient, hessian), imply(loss), params)
    return evaluate!(objective, gradient, hessian, loss, params)
end

function evaluate!(objective, gradient, hessian,
                   sem::Union{SemFiniteDiff, FiniteDiffLossWrappers}, params)
    wrapped = _unwrap(sem)
    obj(p) = _evaluate!(objective_zero(objective, gradient, hessian),
                        nothing, nothing, wrapped, p)
    ...
    # FIXME if objective is not calculated, SemLoss imply states may not correspond to params
