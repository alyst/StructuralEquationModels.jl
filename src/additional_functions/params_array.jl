...
function ParamsArray{T,N}(params_map::AbstractVector{<:AbstractVector{Int}},
    ...
    params_ptr = pushfirst!(accumulate((ptr, inds) -> ptr + length(inds),
                                       params_map, init=1), 1)
    return ParamsArray{T,N}(reduce(vcat, params_map, init=Vector{Int}()), params_ptr,
                            constants, size)
end
...
Base.size(arr::ParamsArray, i::Integer) = size(arr.size, i)
...
function materialize!(dest::AbstractArray{<:Any, N}, src::ParamsArray{<:Any, N},
    ...
    set_zeros && fill!(dest, zero(eltype(dest)))
    if set_constants
...
materialize(arr::ParamsArray, param_values::AbstractVector{T}) where T =
    materialize(T, arr, param_values)
...
function sparse_materialize(::Type{T},
    ...
    nparams(arr) == length(param_values) ||
        throw(DimensionMismatch("Number of values ($(length(param)values))) does not match the number of parameter ($(nparams(arr)))"))
