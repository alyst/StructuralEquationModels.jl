...
start_simple(model::SemLoss; kwargs...) =
    start_simple(observed(model), imply(model); kwargs...)
...
function start_simple(
    ...
    for i in eachindex(start_vals)
        ...
        if length(Si_ind) != 0
            ...
            if c_ind[1] == c_ind[2]
                par = ifelse(c_ind[1] ∈ obs_inds,
                             start_variances_observed,
                             start_variances_latent)
            else
                o1 = c_ind[1] ∈ obs_inds
                o2 = c_ind[2] ∈ obs_inds
                par = ifelse(o1 && o2, start_covariances_observed,
                      ifelse(!o1 && !o2, start_covariances_latent,
                                         start_covariances_obs_lat))
            end
        end

        Ai_ind = param_occurences(A, i)
        if length(Ai_ind) != 0
            iszero(par) || @warn "param[$i]=$(params(ram_matrices, i)) is already set to $par"
            c_ind = C_indices[Ai_ind[1]]
            par = ifelse((c_ind[1] ∈ obs_inds) && !(c_ind[2] ∈ obs_inds),
                         start_loadings, start_regressions)
        end
        ...
        if !isnothing(M)
            ...
            if length(Mi_inds) != 0
                iszero(par) || @warn "param[$i]=$(params(ram_matrices, i)) is already set to $par"
                par = ifelse(Mi_inds[1] ∈ obs_inds,
                             start_mean_observed, start_mean_latent)
            end
        end
